...
class juneincrease: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var junedailyincreases: UILabel!
    @IBOutlet weak var num: UILabel!
    
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        junedailyincreases.isHidden = false
        for date in 1...30 {
            flag = true
            ref = Database.database().reference()
            ref?.child("Stats").child("June").child(String(date)).child("Increase").observe(.value, with: { (snap) in
    ...
    override func viewDidLoad() {
        ...
        junedailyincreases.isHidden = true
    }
