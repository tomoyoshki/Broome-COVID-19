...
class mayincrease: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var maydailyincreases: UILabel!
    @IBOutlet weak var num: UILabel!
    
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        maydailyincreases.isHidden = false
        for date in 1...31{
            flag = true
            ref = Database.database().reference()
            ref?.child("Stats").child("May").child(String(date)).child("Increase").observe(.value, with: { (snap) in
    ...
    override func viewDidLoad() {
        ...
        maydailyincreases.isHidden = true
    }
