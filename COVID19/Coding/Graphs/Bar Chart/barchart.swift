...
class barchart: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var marchdailyincreases: UILabel!
    @IBOutlet weak var num: UILabel!
    
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        flag = true
        marchdailyincreases.isHidden = false
        for date in 15...31 {
            ref = Database.database().reference()
            databaseHandle = ref?.child("Stats").child("March").child(String(date)).child("Increase").observe(.value, with: { (snap) in
    ...
    override func viewDidLoad() {
        ...
        marchdailyincreases.isHidden = true
    }
