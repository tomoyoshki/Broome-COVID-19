...
class aprilincrease: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var aprildailyincreases: UILabel!
    @IBOutlet weak var num: UILabel!
    
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        aprildailyincreases.isHidden = false
        for date in 1...30{
            flag = true
            ref = Database.database().reference()
            ref?.child("Stats").child("April").child(String(date)).child("Increase").observe(.value, with: { (snap) in
    ...
    override func viewDidLoad() {
        ...
        aprildailyincreases.isHidden = true
    }
