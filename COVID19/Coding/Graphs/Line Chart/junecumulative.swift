...
class junecumulative: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var junedate: UILabel!
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        print("cool")
           junedate.isHidden = false
           flag = true
           for date in 1...30 {
               ref = Database.database().reference()
               databaseHandle = ref?.child("Stats").child("June").child(String(date)).child("Active").observe(.value, with: { (snap) in
