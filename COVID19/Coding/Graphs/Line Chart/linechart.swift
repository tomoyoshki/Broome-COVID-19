...
class linechart: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var marchdate: UILabel!
    @IBOutlet weak var cs: UIButton!
    @IBAction func update(_ sender: Any) {
        flag = true
        marchdate.isHidden = false
        for date in 15...31 {
            ref = Database.database().reference()
            databaseHandle = ref?.child("Stats").child("March").child(String(date)).child("Active").observe(.value, with: { (snap) in
