...
class maycumulative: UIViewController, ChartViewDelegate{
    ...
    @IBOutlet weak var maydate: UILabel!
    @IBOutlet weak var cs: UIButton!
     @IBAction func update(_ sender: Any) {
           maydate.isHidden = false
           flag = true
           for date in 1...31 {
               ref = Database.database().reference()
               databaseHandle = ref?.child("Stats").child("May").child(String(date)).child("Active").observe(.value, with: { (snap) in
