...
class lisle: UIViewController {
    ...
    ref = Database.database().reference()
    databaseHandle = ref?.child("City").child("Village of Lisle").child("Cumulative").observe(.value, with: { (snapshot) in
    ...
    ref = Database.database().reference()
    databaseHandle = ref?.child("City").child("Town of Lisle").child("Cumulative").observe(.value, with: { (snapshot) in
