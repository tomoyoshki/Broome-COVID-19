...
import UIKit
import Firebase
class dickinson: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       databaseHandle = ref?.child("City").child("Town of Dickinson").child("Active").observe(.value, with: { (snapshot) in
        ...
        ref = Database.database().reference()
        databaseHandle = ref?.child("City").child("Town of Dickinson").child("Cumulative").observe(.value, with: { (snapshot) in
        ...
        ref = Database.database().reference()
        databaseHandle = ref?.child("City").child("Village of Port Dickinson").child("Cumulative").observe(.value, with: { (snapshot) in
